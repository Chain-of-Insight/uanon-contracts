...
class TestOracle:
    ...
    def oracle_contract(self):
        self.oracle = ContractInterface.create_from(join(dirname(__file__), '../build/contracts/oracle.tz'), shell='sandboxnet')
    ...
    def test_alice_can_create_new_puzzle(self):
        res = self.oracle \
            ...
            .result(storage=self.initial_storage, sender=self.alice)
        ...
        assert 'puzzles' in res.big_map_diff
        assert 1583093350498 in res.big_map_diff['puzzles']
        assert expected == res.big_map_diff['puzzles'][1583093350498]
    ...
    def test_bob_cannot_create_new_puzzle(self):
        with pytest.raises(MichelsonRuntimeError, match='OperationNotAllowed'):
            res = self.oracle \
                .create(id=1583093350498, questions=2, rewards=10, rewards_h='7b15bb3dee5f8891f60cd181ff424012548a9ed5e26721eb9f6518e9dd409d9e') \
                .result(storage=self.initial_storage, sender=self.bob)
    ...
    def test_owner_cannot_create_new_puzzle(self):
        with pytest.raises(MichelsonRuntimeError, match='OperationNotAllowed'):
            res = self.oracle \
                .create(id=1583093350498, questions=2, rewards=10, rewards_h='7b15bb3dee5f8891f60cd181ff424012548a9ed5e26721eb9f6518e9dd409d9e') \
                .result(storage=self.initial_storage, sender=self.owner)
    ...
    def test_cannot_create_existing_puzzle(self):
        self.initial_storage['puzzles'] = {1583093350498: {
            'author': self.alice,
            'claimed': {},
            'questions': 2,
            'id': 1583093350498,
        ...
        with pytest.raises(MichelsonRuntimeError, match='Puzzle already exists'):
            res = self.oracle \
                .create(id=1583093350498, questions=2, rewards=10, rewards_h='7b15bb3dee5f8891f60cd181ff424012548a9ed5e26721eb9f6518e9dd409d9e') \
                .result(storage=self.initial_storage, sender=self.alice)
    ...
    #                 'rewards_h': '7b15bb3dee5f8891f60cd181ff424012548a9ed5e26721eb9f6518e9dd409d9e'}
    #
    #     assert 1583093350498 in res.big_map_diff
    #     assert storage[1583093350498] == res.big_map_diff[1583093350498]
    #     assert 1583258505553 in res.big_map_diff
    #     assert expected == res.big_map_diff[1583258505553]
    #
    ...
    #
    #     assert 1583093350498 in res.big_map_diff
    #     assert expected == res.big_map_diff[1583093350498]
    #
    #
